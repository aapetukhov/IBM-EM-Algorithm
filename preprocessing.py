...
def extract_sentences(filename: str) -> Tuple[List[SentencePair], List[LabeledAlignment]]:
    ...
    for sentences in root.findall('s'):
        ...
        sure_alignments = [(int(pair.split('-')[0]), int(pair.split('-')[1])) for pair in sure_pairs.split()] if sure_pairs else []
        
        possible_text = sentences.find('possible').text
        possible_alignments = [(int(pair.split('-')[0]), int(pair.split('-')[1])) for pair in possible_text.split()] if possible_text else []
...
def tokenize_sents(sentence_pairs: List[SentencePair], source_dict, target_dict) -> List[TokenizedSentencePair]:
    ...
    for sentence_pair in sentence_pairs:
        tokenized_source = [source_dict.get(token) for token in sentence_pair.source if token in source_dict]
        tokenized_target = [target_dict.get(token) for token in sentence_pair.target if token in target_dict]

        if not tokenized_source or not tokenized_target:
            continue

        tokenized_sentence_pairs.append(TokenizedSentencePair(source_tokens=np.array(tokenized_source, dtype=np.int32),
...
def unicode_normalize(sentence_pairs: List[SentencePair], encoding: str = 'NFC') -> List[SentencePair]:
    ...
    for sentence_pair in sentence_pairs:
        normalized_sentence_pairs.append(SentencePair(source=[unicodedata.normalize(encoding, word.lower()) for word in sentence_pair.source],
                                                       target=[unicodedata.normalize(encoding, word.lower()) for word in sentence_pair.target]))
...
def extract_sentences_from_gzip(file_prefix: str, encoding: str = 'NFC') -> List[SentencePair] :
    ...
    en_sentences = {s.get('id'): ' '.join(w.text for w in s.iter('w')) for s in parse_gzip(en_file).iter('s') if s.get('id') in en_ids}
    cs_sentences = {s.get('id'): ' '.join(w.text for w in s.iter('w')) for s in parse_gzip(cs_file).iter('s') if s.get('id') in cs_ids}
    
    #сразу переводим в lowercase
    all_sentences = [SentencePair(source=unicodedata.normalize(encoding, en_sentences[en_id].lower()).split(),
                                  target=unicodedata.normalize(encoding, cs_sentences[cs_id].lower()).split()) for en_id, cs_id in alignments if en_id in en_sentences and cs_id in cs_sentences]
...
def extract_prefixes(directory: str) -> List[str]:
    ...
    return list(map(lambda filename: filename[:-14], glob.glob(f'{directory}/**/*-salign.xml.gz', recursive=True)))
