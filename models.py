...
class WordAligner(BaseAligner):
    def __init__(self, num_source_words, num_target_words, num_iters):
        ...
        self.translation_probs = np.full((num_source_words, num_target_words), 1 / num_target_words, dtype=np.float32) #theta matrix
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence_pair in parallel_corpus:
            ...
            posteriors.append( translation_probs / np.sum(translation_probs, axis=0, keepdims=True))
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for posterior, sentence_pair in zip(posteriors, parallel_corpus):
            elbo += np.sum(posterior * ((np.log(np.maximum(self.translation_probs[get_indices(sentence_pair)], 1e-10))) - np.log(np.maximum(posterior * len(sentence_pair.source_tokens), 1e-10))))
    ...
    def align(self, sentences):
        ...
        for posterior in posteriors:
            alignments.append([(source_pos, target_pos) for target_pos, source_pos in enumerate(posterior.argmax(axis=0) + 1, 1)])
        return alignments
...
class WordPositionAligner(WordAligner):
    ...
    def _get_probs_for_lengths(self, src_length: int, tgt_length: int):
        ...
        if (src_length, tgt_length) not in self.alignment_probs.keys():
            self.alignment_probs[(src_length, tgt_length)] = np.full(shape=(src_length, tgt_length), 
                                                                     fill_value=1/src_length)
    ...
    def _e_step(self, parallel_corpus):
        ...
        for sentence_pair in parallel_corpus:
            translation_probs = self.translation_probs[get_indices(sentence_pair)]
            alignment_probs = self._get_probs_for_lengths(len(sentence_pair.source_tokens), len(sentence_pair.target_tokens))
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        elbo = 0
        for posterior, sentence_pair in zip(posteriors, parallel_corpus):
            elbo += np.sum(posterior *\
                            ((np.log(np.maximum(self.translation_probs[get_indices(sentence_pair)] *\
                                                            self._get_probs_for_lengths(len(sentence_pair.source_tokens), len(sentence_pair.target_tokens)), 1e-10))) -\
...
class WordPositionAlignerWithPriors(WordPositionAligner):
    def __init__(self, num_source_words, num_target_words, num_iters, initial_lambda: float = 4.0, learning_rate: float = 0.01, grad_reg: float = 1e-1):
        super().__init__(num_source_words, num_target_words, num_iters)
        ...
        if (src_length, tgt_length) not in self.alignment_probs:
            ...
            for i in range(tgt_length):
                ...
                for j in range(src_length):
                    log_probs[j, i] = self.lambda_ * self.h(i, j, tgt_length, src_length) - log_Z
    ...
    def update_lambda(self, parallel_corpus, posteriors):
        ...
        for idx, sentence_pair in enumerate(parallel_corpus):
            ...
            for i in range(m):
                ...
                gradient += (expected_h 
                            - (t_j_up * self.h(i, j_up, m, n) + t_j_down * self.h(i, j_down, m, n)) / Z_lambda)
    ...
    def _e_step(self, parallel_corpus):
        posteriors = []
        for sentence_pair in parallel_corpus:
            log_translation_probs = np.log(self.translation_probs[get_indices(sentence_pair)])
            log_alignment_probs = np.log(self._get_probs_for_lengths(len(sentence_pair.source_tokens), len(sentence_pair.target_tokens)))
            log_posterior = log_translation_probs + log_alignment_probs
            log_posterior -= np.max(log_posterior, axis=0, keepdims=True)  # For numerical stability
    ...
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            ...
            for i in range(m):
                ...
                t_j_up = np.exp(self.lambda_ * self.h(i, j_up, m, n))
                t_j_down = np.exp(self.lambda_ * self.h(i, j_down, m, n))
                lambda_gradient += (expected_h - (t_j_up * self.h(i, j_up, m, n) + t_j_down * self.h(i, j_down, m, n)) / Z_lambda)
        ...
        # Update lambda
        self.lambda_ = np.clip(self.lambda_ + self.learning_rate * lambda_gradient, -500, 100)  # Clip lambda to avoid extreme values
